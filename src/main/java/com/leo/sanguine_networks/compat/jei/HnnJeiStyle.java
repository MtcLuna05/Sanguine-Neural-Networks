...
final class HnnJeiStyle implements IDrawable {
    ...
    HnnJeiStyle(IGuiHelper helper) {
        original = helper.createDrawable(new ResourceLocation("sanguine_networks", "textures/jei/virtual_sacrificer.png"), 0, 0, 116, 43);
    }
    ...
    static void drawProgress(GuiGraphics graphics) {
        ...
        int width = Mth.ceil(35F * (mc.level.getGameTime() % 40 + mc.getFrameTime()) / 40);
        graphics.blit(new ResourceLocation("sanguine_networks:textures/jei/virtual_sacrificer.png"), 52, 9, 0, 43, width, 6, 256, 256);
    }
}
